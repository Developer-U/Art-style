<?php
/**
 * Template part for displaying Fast Zakaz block
 *
 */
$page_id = get_the_ID();
$socials = get_field('social_icons', 'options');
$cta_text = get_field('cta_text', $page_id);
$cta_before_text = get_field('cta_before_text', $page_id);

if ($socials['whatsapp'] || $socials['telegram'] || $socials['vk']) { ?>
    <div class="hero__zakaz fast-zakaz">
        <?php if ($cta_before_text) { ?>
            <h4 class="fast-zakaz__before" data-aos="fade-right" data-aos-offset="50" data-aos-delay="50"
                data-aos-duration="1200" data-aos-easing="ease-in-out" data-aos-mirror="true" data-aos-once="false">
                <?php echo $cta_before_text; ?>
            </h4>
        <?php }

        if ($cta_text) { ?>
            <a class="<?php if (is_front_page()) { ?>fast-zakaz__text <?php } else { ?>button red-btn <?php } ?> col-auto"
                href="#" data-popup-open="fast-popup">
                <?php echo $cta_text; ?>
            </a>
        <?php } else { ?>
            <a class="<?php if (is_front_page()) { ?>fast-zakaz__text <?php } else { ?>button red-btn <?php } ?> col-auto"
    ...
    <!-- Попап Быстрый заказ -->
    <section data-popup="fast-popup" class="popup">
        <div class="popup__wrapper">
            <div class="popup__cont">
                <button data-popup-close="fast-popup" class="popup__del">
                ...
                <h3 class="popup__heading">
                    Напишите нам в любимом мессенджере:
                </h3>
